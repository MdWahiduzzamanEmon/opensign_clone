...
function RenderPdf(props) {
  ...
  const posWidth = (pos, signYourself) => {
    ...
    // If widget position data exists
    const widgetPos = props.widgetPositions[pos.key] || {};

    // Adjust the width based on widget position
    return (
      widgetPos.width ||
  ...
  const posHeight = (pos, signYourself) => {
    ...
    // If widget position data exists
    const widgetPos = props.widgetPositions[pos.key] || {};
    // Adjust the height based on widget position
    return (
      widgetPos.height ||
      (signYourself ? posHeight * props.scale * containerScale : posHeight * containerScale)
    );
  };
  ...
  // Memoize the PDF document rendering
  const pdfDocument = useMemo(
    () => (
      <>
        <Document
          ...
          onClick={() => props.setSelectWidgetId && props.setSelectWidgetId('')}
          file={pdfDataBase64}
        >
          <Page
            key={props.index}
    ...
    [t, props, pdfDataBase64],
  );
  return (
    <>
      {props.successEmail && <Alert type={'success'}>{t('success-email-alert')}</Alert>}
      {isMobile ? (
        <RSC
          style={{
            position: 'relative',
        ...
        >
          <Box
            ...
            style={{
              width: props.containerWH?.width && props.containerWH?.width * props.scale,
            }}
            ref={props.drop}
            id="container"
          >
            {props.containerWH?.width &&
              ...
              (props.pdfRequest || props.isSelfSign
                ? props.signerPos?.map((data, key) => {
                    return <React.Fragment key={key}>{checkSignedSigners(data)}</React.Fragment>;
                  })
                                    ...
                                    placeData.pos.map((pos) => {
                                      return (
                                        <React.Fragment key={pos.key}>
                                          <Placeholder
                                            pos={pos}
                                            widgetPosition={props.widgetPositions[pos.key]}
                                            setIsPageCopy={props.setIsPageCopy}
                                            setSignKey={props.setSignKey}
                                            handleDeleteSign={props.handleDeleteSign}
                                            setIsStamp={props.setIsStamp}
                              ...
                              return (
                                pos && (
                                  <Placeholder
                                    ...
                                    handleTabDrag={props.handleTabDrag}
                                    handleStop={props.handleStop}
                                    handleSignYourselfImageResize={handleSignYourselfImageResize}
                                    index={props.index}
                                ...
                                <React.Fragment key={pos.key}>
                                  <Placeholder
                                    ...
                                    handleStop={(event, dragElement) =>
                                      props.handleStop(event, dragElement, pos.type)
                                    }
                                    handleSignYourselfImageResize={handleSignYourselfImageResize}
                                    index={props.index}
                                    xyPosition={props.xyPosition}
