...
import { emailRegex, radioButtonWidget, textInputWidget, textWidget } from '../../config';
import { getMonth, getYear, onChangeHeightOfTextArea, onChangeInput } from '../../constant/constant';
const textWidgetCls =
  ...
  switch (type) {
      ...
      return props.pos.SignUrl ? (
        ...
        <div className={widgetCls}>
          {props.pos.type && (
            <div
              style={{
                fontSize: props.pos ? props.calculateFontsize(props.pos) : '11px',
              }}
      ...
      return (
        <div style={{ zIndex: props.isSignYourself && '99' }}>
          {props.pos.options?.values?.map((data, ind) => {
            return (
              <div key={ind} className="select-none-cls flex items-center text-center gap-0.5">
                <input
                  ...
                  disabled={
                    props.isNeedSign &&
                    (props.pos.options?.isReadOnly || props.data?.signerObjId !== props.signerObjId)
                  }
                  type="checkbox"
                  ...
                  onChange={(e) => {
                    if (e.target.checked) {
                      if (!props.isPlaceholder) {
                        const maxRequired = props.pos.options?.validation?.maxRequiredCount;
                        const maxCountInt = maxRequired && parseInt(maxRequired);

                        if (maxCountInt > 0) {
                          if (selectedCheckbox && selectedCheckbox?.length <= maxCountInt - 1) {
    ...
    case 'dropdown':
      return props.data?.signerObjId === props.signerObjId ? (
        ...
        >
          ...
          <option style={{ fontSize: fontSize, color: fontColor }} value="" disabled hidden>
            {props?.pos?.options?.name}
          </option>

          {props.pos?.options?.values?.map((data, ind) => {
            return (
              <option style={{ fontSize: fontSize, color: fontColor }} key={ind} value={data}>
                {data}
              </option>
            );
          })}
        ...
        <div style={textWidgetStyle} className="select-none-cls flex justify-between items-center">
          {props.pos?.options?.name ? props.pos.options.name : widgetTypeTranslation}
    ...
    case 'date':
      return props.isSignYourself ||
        ...
        <div style={textWidgetStyle} className="select-none-cls overflow-hidden">
          <span>
            {props.selectDate
              ? props.selectDate?.format
              : props.pos?.options?.validation?.format
    ...
    default:
      return props.pos.SignUrl ? (
        <div className="pointer-events-none">
          <img alt="image" draggable="false" src={props.pos.SignUrl} className="w-full h-full " />
        </div>
      ) : (
        <div className={widgetCls}>
