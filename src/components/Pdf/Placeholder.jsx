...
function Placeholder(props) {
  ...
  console.log(props.widgetPosition, 'widgetPosition');

  const navigator = useNavigate();
  const [startDate, setStartDate] = useState(
    props?.pos?.options?.response
      ? getDefaultDate(props?.pos?.options?.response, props.pos?.options?.validation?.format)
      : new Date(),
  );
  ...
  const handleWidgetIdandPopup = () => {
    ...
    if (props.isOpenSignPad && !props.isDragging) {
      if (props?.ispublicTemplate) {
        ...
        if (props?.isNeedSign) {
          ...
          const getCurrentSignerPos = props.xyPosition.find((x) => x.Id === props.uniqueId);
        ...
        if (['signature', 'stamp', 'image', 'initials'].includes(props.pos.type)) {
    ...
    } else if (props.isPlaceholder && !props.isDragging && props.pos.type !== textWidget) {
      if (props.pos.key === props.selectWidgetId) {
        props.handleLinkUser(props.data.Id);
        props.setUniqueId(props.data.Id);
        const checkIndex = props.xyPosition.findIndex((data) => data.Id === props.data.Id);
        props.setIsSelectId(checkIndex || 0);
      }
    } else if (!props.pos.type) {
      if (!props.pos.type && props.isNeedSign && props.data.signerObjId === props.signerObjId) {
        props.setIsSignPad(true);
        props.setSignKey(props.pos.key);
        props.setIsStamp(props.pos.isStamp);
      } else if (
  ...
  const handleOnClickPlaceholder = () => {
    ...
    if (props.isApplyAll) {
      ...
      if (['signature', 'initials', 'stamp'].includes(props.pos.type)) {
        ...
        if (existSignPosition.type === props.pos.type && existSignPosition && isCopySignature) {
          props.setCurrWidgetsDetails && props.setCurrWidgetsDetails(props.pos);
  ...
  const handleCopyPlaceholder = (e) => {
    ...
    if (['signature', textInputWidget, textWidget, 'stamp', 'initials'].includes(props.pos.type)) {
  ...
  const handleSaveDate = (data, isDateChange) => {
    ...
    if (data?.format === 'dd-MM-yyyy') {
      ...
      date = moment(newDate.getTime()).format(changeDateToMomentFormat(data?.format));
    }
        ...
        <>
          {(props.isPlaceholder || props.isSignYourself || props.isSelfSign) && (
            ...
            */}
              ...
              {props.pos.type !== textWidget && !props.isSignYourself && !props.isSelfSign && (
                <i
    ...
    <>
      ...
      props.pos.type === textWidget ? (
        ...
        <Rnd
          ...
          lockAspectRatio={
            !props.isFreeResize &&
            ![textWidget, 'email', 'name', 'company', 'job title', textInputWidget].includes(
              props.pos.type,
            ) &&
            (props.pos.Width
            ...
            zIndex:
              props.pos.type === 'date'
                ...
                : props?.pos?.zIndex
                  ? props.pos.zIndex
                  : '5',
            opacity: props.isNeedSign && props.data?.Id !== props?.uniqueId && '0.4',
        ...
        >
          {props.pos.key === props.selectWidgetId &&
          ((props.isShowBorder && ![radioButtonWidget, 'checkbox'].includes(props.pos.type)) ||
            (props?.isAlllowModify && !props?.assignedWidgetId.includes(props.pos.key))) ? (
            <BorderResize
              right={-12}
              top={-11}
      ...
      {/* <ModalUi isOpen={isDateModal} title={t('widget-info')} showClose={false}>
        <div className="h-[100%] p-[20px]">
          <div className="flex flex-row items-center">
            ...
            <span className="text-xs text-gray-400 ml-1">{selectDate.format}</span>
          </div>
          <div className="flex items-center mt-4 md:mt-2">
            ...
            <div className="flex flex-row gap-1 items-center ml-2 md:ml-4 ">
              <span>{t('color')}: </span>
              <select
                value={props.fontColor || clickonWidget.options?.fontColor || 'black'}
              ...
              <span
                style={{
                  background: props.fontColor || props.pos.options?.fontColor || 'black',
                }}
